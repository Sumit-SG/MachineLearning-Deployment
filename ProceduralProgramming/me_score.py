...
if __name__=="__main__":
    ...
    X_test = data_prep(data=X_test)

    pred_acc, pred_roc =pf.predict_model(data=X_test,model_path=config.MODEL_PATH)

    print(f'Test Accuracy:{accuracy_score(y_test,pred_acc):0.5f}')
    print(f'Test ROC:{roc_auc_score(y_test,pred_roc[:,1]):0.5f}')
    print()
